...
<template>
    ...
    <RouterLink :to="{name: 'CardDetails', params: {id: props.employee._id}}">
        ...
        <img :src="faker.image.urlLoremFlickr({width: 640, height: 480, category: 'bird'})" class="rounded-t-xl"/>
        <div class="p-4">
            <p class="mb-2 text-2xl font-bold">{{ props.employee.firstName }} {{ props.employee.lastName }}</p>
            <p class="mb-3 font-normal text-grey-700">{{ props.employee.title }}</p>
            <p class="mb-3 font-normal text-grey-700">{{ props.employee.quote }}</p>
        </div>
    </div>
    </RouterLink>
